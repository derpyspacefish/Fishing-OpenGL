...
bool Model::loadModel(char* filename)
{
	...
	vector<unsigned int> faces; // find
	...
	while (true)
	{
		...
		else // Parse
		{
			...
			{
				unsigned int face[9];
				int matches = fscanf(file, "%d/%d/%d %d/%d/%d %d/%d/%d\n", &face[0], &face[1], &face[2], //contains values to find the x,y, and z components of the first vertex in a face
																			&face[3], &face[4], &face[5], //contains values to find the x,y, and z components of the second vertex in a face
																			&face[6], &face[7], &face[8]); //contains values to find the x,y, and z components of the third vertex in a face
				if (matches != 9)
				{
	...
	for (int i = 0; i < faces.size(); i += 3)
	{
		...
		vertXYZ.push_back(verts[faces[i]].getX());
		vertXYZ.push_back(verts[faces[i]].getY());
		vertXYZ.push_back(verts[faces[i]].getZ());

		texXY.push_back(texCs[faces[i+1]].getX());
		texXY.push_back(texCs[faces[i+1]].getY());

		normXYZ.push_back(norms[faces[i+2]].getX());
		normXYZ.push_back(norms[faces[i+2]].getY());
		normXYZ.push_back(norms[faces[i+2]].getZ());
	}
